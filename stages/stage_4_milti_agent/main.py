...
async def check_routing(state: LegalState) -> dict:
    ...
    def keyword_routing() -> dict:
        needs_tax = any(kw in question_lower for kw in ["tax", "irs", "thuế", "fbar", "fatca"])
        needs_compliance = any(
            kw in question_lower
            for kw in ["compliance", "sec", "regulation", "regulatory", "sox", "consent"]
        )
        needs_privacy = any(
            kw in question_lower
            for kw in ["data", "privacy", "gdpr", "personal information", "user data", "dữ liệu"]
        )
        return {
            "needs_tax": needs_tax,
    ...
    try:
        ...
        fallback = keyword_routing()
        print(f"  [Node: check_routing] LLM routing failed; using keyword routing: {exc}")
        print(
            "  [Node: check_routing] "
            f"needs_tax={fallback['needs_tax']}, "
            f"needs_compliance={fallback['needs_compliance']}, "
            f"needs_privacy={fallback['needs_privacy']}"
        )
        return fallback
    ...
    needs_privacy = keyword_routing()["needs_privacy"]
    print(
        "  [Node: check_routing] "
...
async def call_tax_specialist(state: LegalState) -> dict:
    ...
    try:
        ...
        message = f"[Tax analysis unavailable: {exc}]"
        print(f"  [Node: call_tax_specialist] Failed: {exc}")
        return {"tax_result": message}
...
async def call_compliance_specialist(state: LegalState) -> dict:
    ...
    try:
        ...
        message = f"[Compliance analysis unavailable: {exc}]"
        print(f"  [Node: call_compliance_specialist] Failed: {exc}")
        return {"compliance_result": message}
...
async def call_privacy_specialist(state: LegalState) -> dict:
    """Privacy specialist sub-agent for data protection and GDPR issues."""
    print("\n  [Node: call_privacy_specialist] Privacy specialist agent starting...")
    llm = get_llm()

    prompt = f"""You are a specialist privacy attorney with expertise in GDPR, CCPA,
data protection, data breach notification, consent requirements, and privacy rights.

Original question: {state['question']}
General legal analysis: {state.get('law_analysis', 'N/A')}

Analyze the privacy and GDPR/data protection issues if any. Keep your response under 200 words.
"""

    try:
        result = await llm.ainvoke([HumanMessage(content=prompt)])
    except Exception as exc:
        message = f"[Privacy analysis unavailable: {exc}]"
        print(f"  [Node: call_privacy_specialist] Failed: {exc}")
        return {"privacy_result": message}
...
QUESTION = (
    "If a tech company breaks a contract, avoids taxes, and leaks user data without consent, "
